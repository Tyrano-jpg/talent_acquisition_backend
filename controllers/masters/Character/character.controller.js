...
export const addCharacter = catchAsync(async (req, res, next) => {
  ...
  const maxNumber = await characterModel.aggregate([
    {
      $group: {
        _id: null,
        max: { $max: '$sr_no' },
...
export const updateCharacter = catchAsync(async (req, res, next) => {
  ...
  const updateCharacterData = await characterModel.updateOne(
    { _id: id },
    {
      $set: characterData,
    }
  ...
  if (
    !updateCharacterData.acknowledged ||
    updateCharacterData.modifiedCount <= 0
  ) {
