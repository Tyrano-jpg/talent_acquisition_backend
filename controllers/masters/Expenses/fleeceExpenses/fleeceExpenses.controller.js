...
export const fleeceExpenses_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const List_fleece_invoice_details =
      await fleece_inventory_invoice_modal.aggregate(aggregate_stage);
...
export const fleeceExpenses_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const fleeceExpense_item_by_invoice =
      await fleece_inventory_items_modal.aggregate(aggregate_stage);
    const fleeceExpense_invoice = await fleece_inventory_invoice_modal.findOne({
      _id: invoice_id,
    });
...
export const add_fleeceExpenses = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const updateFleeceItemsExpenses =
      await fleece_inventory_items_modal.aggregate([
        {
          $match: {
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
          },
