...
export const plywoodExpenses_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const List_plywood_invoice_details =
      await plywood_inventory_invoice_details.aggregate(aggregate_stage);

    const totalCount = await plywood_inventory_invoice_details.countDocuments({
      ...match_query,
    });
...
export const plywoodExpenses_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const plywoodExpense_item_by_invoice =
      await plywood_inventory_items_details.aggregate(aggregate_stage);
    const plywoodExpense_invoice =
      await plywood_inventory_invoice_details.findOne({ _id: invoice_id });
...
export const add_plywoodExpenses = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const invoiceDetails = await plywood_inventory_invoice_details.findOne({
    ...
    const updatePlywoodItemsExpenses =
      await plywood_inventory_items_details.aggregate([
        {
          $match: {
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
          },
        ...
        {
          $merge: {
            into: 'plywood_inventory_items_details',
            whenMatched: 'merge',
          },
        },
      ]);
