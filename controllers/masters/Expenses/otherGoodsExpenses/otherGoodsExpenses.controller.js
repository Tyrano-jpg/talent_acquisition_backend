...
export const otherGoodsExpenses_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const List_otherGoods_invoice_details =
      await othergoods_inventory_invoice_details.aggregate(aggregate_stage);

    const totalCount =
      await othergoods_inventory_invoice_details.countDocuments({
        ...match_query,
      });
...
export const otherGoodsExpenses_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const otherGoodsExpense_item_by_invoice =
      await othergoods_inventory_items_details.aggregate(aggregate_stage);
    const otherGoodsExpense_invoice =
      await othergoods_inventory_invoice_details.findOne({ _id: invoice_id });
...
export const add_otherGoodsExpenses = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const updateOtherGoodsItemsExpenses =
      await othergoods_inventory_items_details.aggregate([
        {
          $match: {
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
          },
